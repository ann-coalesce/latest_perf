...
def get_nav_test(end_time: Optional[str] = None) -> pd.DataFrame:
    """
    Get NAV data for portfolio managers at current/specified time and period starts.
    ...
    try:
        ...
        # Pivot and clean the data
        nav_pivot = _pivot_and_clean_data_test(
            data=nav_data,
            value_column='value',
            type_column='data_type',
            main_column='nav',
...
def get_bal_from_nav(end_time: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        # Pivot and clean the data
        balance_pivot = _pivot_and_clean_data(
            data=balance_data,
            value_column='value',
            type_column='data_type',
            main_column='balance',
...
def get_bal_from_nav_test(end_time: Optional[str] = None) -> pd.DataFrame:
    """
    Get balance data for portfolio managers at current/specified time and period starts.
...
if __name__ == "__main__":
    ...
    print("Bal Data:")
    print(df)
    sheet_utils.set_dataframe(df=df, url='https://docs.google.com/spreadsheets/d/1CYfRlJ--aQknxzSjdMft4puFg-vVb8VPo-3jlFVlqBU/edit?gid=1948587986#gid=1948587986', sheet_name='bal_test')
