...
class PerformanceMetricsProcessor:
    ...
    def _get_base_data(self, end_time: Optional[datetime] = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        bal_merged_df = bal_merged_df[bal_merged_df['pm'] == 'ctaalphasierra'].copy()
        nav_merged_df = nav_merged_df[nav_merged_df['pm'] == 'ctaalphasierra'].copy()
        transfer_df = transfer_df[transfer_df['pm'] == 'ctaalphasierra'].copy()
    ...
    def _replace_aggregated_pnl(self, df, pm_data, pnl_columns='ITD_PnL', 
        ...
        if validate:
            validation_passed = self._validate_pnl_aggregation(
                ...
                custom_thresholds={'sp1-gross': 0.60},  # Can be overridden
